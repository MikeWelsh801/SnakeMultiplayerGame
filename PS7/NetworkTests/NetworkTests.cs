...
namespace NetworkUtil
{
    ...
    public class NetworkTests
    {
        ...
        [DataRow(false)]
        [DataTestMethod]
        public void TestSendteenyMessage(bool clientSide)
        {
            SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
            ...
            for (int i = 0; i < 2; i++)
                sb.Append(i + "helllooooo");
            Networking.Send(testRemoteSocketState.TheSocket, sb.ToString());

            NetworkTestHelper.WaitForOrTimeout(() => testLocalSocketState.GetData().Length == sb.Length, NetworkTestHelper.timeout);

            Assert.AreEqual(sb.ToString(), testLocalSocketState.GetData());
        }

        [DataRow(true)]
        [DataRow(false)]
        [DataTestMethod]
        public void TestSendandCloseteenyMessage(bool clientSide)
        {
            SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
            ...
            for (int i = 0; i < 2; i++)
                sb.Append(i + "helllooooo");
            Networking.SendAndClose(testRemoteSocketState.TheSocket, sb.ToString());

            NetworkTestHelper.WaitForOrTimeout(() => testLocalSocketState.GetData().Length == sb.Length, NetworkTestHelper.timeout);

            // Check message got through
            Assert.AreEqual(sb.ToString(), testLocalSocketState.GetData());

            // Check remote socket was closed
            Assert.IsFalse(testRemoteSocketState.TheSocket.Connected);
        }

        [TestMethod]
        public void TestBogusIP()
        {
            void NetworkAction(SocketState s)
            {
                if (s.ErrorOccurred)
                    Assert.IsTrue(true);
                return;
            }
            void Foo(SocketState a)
            {
                return;
            }
            Networking.StartServer(Foo, 2201);

            Networking.ConnectToServer(NetworkAction, "boooooo", 2201);
        }

        [TestMethod]
        public void TestConnectThenClose()
        {
            NetworkTestHelper.SetupSingleConnectionTest(out testListener, out testLocalSocketState, out testRemoteSocketState);
            Assert.IsNotNull(testLocalSocketState);
            Assert.IsNotNull(testRemoteSocketState);
            ...
            // Check that the listener socket is closed
